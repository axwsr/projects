...
onMounted(() => {
    ...
    if (savedDarkTheme !== null) {
        const isDarkTheme = savedDarkTheme === 'true';
        layoutConfig.darkTheme.value = isDarkTheme;
        const newThemeName = isDarkTheme ? layoutConfig.theme.value.replace('light', 'dark') : layoutConfig.theme.value.replace('dark', 'light');
        onChangeTheme(newThemeName, isDarkTheme);
    }
});
</script>
